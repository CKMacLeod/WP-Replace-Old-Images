 ...
 * Description: A WordPress plug-in for replacing old, broken, and unwanted images with a user-adjustable default image, according to date or file location
 * Version: 1.0
 * Author: CK MacLeod
...
<!--            check functions if any of "name"-->
            <tr valign="top">
                ...
                <td><input type="text" name="roi_options[roi_cut_off_date]" value="<?php echo esc_attr($roi_options['roi_cut_off_date']); ?>" /><br><label>Any readable full date format... </label></td>
            </tr>
            <tr valign="top">
                <th scope="row">Image URL   </th>
                <td><input type="text" size="75" name="roi_options[roi_image_url]" value="<?php echo esc_attr($roi_options['roi_image_url']); ?>" /><br><label>Use full URL (http://... etc.) </label><p>Current Image: </p>
                <img src="<?php echo esc_attr($roi_options['roi_image_url']); ?>"></td>
            </tr>
            <tr valign="top">
                <th scope="row">First String Match (Host Name)  </th>
                <td><input type="text" name="roi_options[roi_match_1]" value="<?php echo esc_attr($roi_options['roi_match_1']); ?>" /></td>
            </tr>
            <tr valign="top">
                <th scope="row"> Second String Match (Directory)  </th>
                <td><input type="text" name="roi_options[roi_match_2]" value="<?php echo esc_attr($roi_options['roi_match_2']); ?>" /></td>
            </tr>        
        </table>
...
function replace_old_images($content) {
    ...
    if ($post_date > $cut_off_date) {
        ...
        //add matching-extensions-only if desired
        //currently set to replace most common image types only
        //test for importance of $content = str_replace(']]>', ']]&gt;', $content);
